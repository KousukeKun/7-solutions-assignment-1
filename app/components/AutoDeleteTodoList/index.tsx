...
function AutoDeleteTodoList(props: AutoDeleteTodoListProps) {
  ...
  const [categoryTodoList, setCategoryTodoList] = useState<Record<Category, TodoItem[]>>(initCategoryTodo);
  ...
  const moveBackToAllTodoList = useCallback((item: TodoItem) => {
    ...
    setAllTodoList(prevAllTodoList => {
      ...
      if (prevAllTodoList.some(todo => todo.type === item.type && todo.name === item.name)) {
